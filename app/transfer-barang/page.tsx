  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editing && form.id) {
        ...
        if (!originalTransfer) throw new Error('Transfer asli tidak ditemukan')
        
        const newJumlah = parseFloat(item.jumlah)
        const oldJumlah = originalTransfer.jumlah
        ...
        // Update gudang if transfer is completed and quantity changed
        if (originalTransfer.status === 'completed' && jumlahDiff !== 0) {
          const peminjamBranch = branches.find(b => b.id_branch === originalTransfer.cabang_peminjam_id)
          const tujuanBranch = branches.find(b => b.id_branch === originalTransfer.cabang_tujuan_id)
          ...
          if (peminjamBranch && tujuanBranch) {
            ...
            // Recalculate totals
            const transferDate = originalTransfer.tgl_barang_sampai || originalTransfer.tgl_pinjam
            await recalculateGudangTotals(originalTransfer.id_product, peminjamBranch.kode_branch, transferDate)
            await recalculateGudangTotals(originalTransfer.id_product, tujuanBranch.kode_branch, transferDate)
          }
        }
        ...
        <div className="p-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Transfer Barang</h1>
            </div>
            <div className="flex gap-2 w-full sm:w-auto">
              <button
                onClick={() => setShowFilter(!showFilter)}
                className="flex items-center justify-center gap-2 px-3 sm:px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 flex-1 sm:flex-none"
              >
          ...
          <div className="bg-white rounded-lg shadow">
            ...
            <div className="md:hidden">
              {loading ? (
                ...
                transfers.map((transfer) => (
                  <div key={transfer.id} className="p-4 border-b border-gray-200 last:border-b-0">
                    ...
                    <div className="space-y-2 mb-3">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium">{transfer.cabang_peminjam}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                      ...
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium text-right">{transfer.product_name}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                      ...
                      {transfer.tgl_barang_sampai && (
                        <div className="flex justify-between text-sm">
                          <span className="text-gray-500">Tgl Sampai:</span>
                          <span className="font-medium">{new Date(transfer.tgl_barang_sampai).toLocaleDateString('id-ID')}</span>
                        </div>
                      )}
                    </div>
              ...
              <div className="p-4 sm:p-6">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="bg-blue-50 p-3 sm:p-4 rounded-lg">
                    <h3 className="text-base sm:text-lg font-semibold text-blue-800 mb-3 sm:mb-4">Informasi Transfer</h3>
                  ...
                  <div className="bg-gray-50 p-3 sm:p-4 rounded-lg">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-3 sm:mb-4 gap-2">
                      <h3 className="text-base sm:text-lg font-semibold text-gray-800">Daftar Barang</h3>
                      <button
                        type="button"
                        onClick={addItem}
                          ...
                          <div>
                            ...
                            <div className="relative">
                              <input
                                ...
                                className="w-full p-2 sm:p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 pr-12 sm:pr-16 text-sm sm:text-base"
                                placeholder="0.00"
                                required
                              />
                              {item.stockInfo && (
                                <div className="absolute right-2 sm:right-3 top-1/2 transform -translate-y-1/2">
                                  <span className="text-xs sm:text-sm text-gray-500">{item.stockInfo.unit}</span>
                                </div>
                              )}
                            </div>
                            {item.stockInfo && (
                  ...
                  <div className="flex flex-col sm:flex-row justify-end gap-3 sm:gap-4 pt-4 sm:pt-6 border-t border-gray-200">
                    <button
                      type="button"
                      onClick={resetForm}
                      className="px-4 sm:px-6 py-2 sm:py-3 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors font-medium text-sm sm:text-base order-2 sm:order-1"
                    >
                      Batal
                    </button>
                    <button
                      type="submit"
                      className="px-4 sm:px-6 py-2 sm:py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium text-sm sm:text-base order-1 sm:order-2"
                    >
